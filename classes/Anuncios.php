...
class Anuncios
{
	public function getTotalAnuncios($filtros)
	{
		...
		$fitrostring = array('1=1');
		if(!empty($filtros['categoria']))
		{
			$fitrostring[] = 'anuncios.id_categoria = :id_categoria';
		}

		if(!empty($filtros['preco']))
		{
			$fitrostring[] = 'anuncios.valor BETWEEN :preco1 AND :preco2';
		}

		if(!empty($filtros['estado']))
		{
			$fitrostring[] = 'anuncios.estado = :estado';
		}
		
		$sql = $pdo->prepare("SELECT count(*) as c FROM anuncios WHERE ".implode(' AND ', $fitrostring));
	...
	public function getUltimosAnuncios($page, $perPage, $filtros)
	{
		...
		$sql = "SELECT *,(select anuncios_imagens.url from anuncios_imagens where anuncios_imagens.id_anuncio = anuncios.id limit 1) as url,(select categorias.nome from categorias where categorias.id = anuncios.id_categoria) as categoria FROM anuncios WHERE ".implode(' AND ', $fitrostring)." ORDER BY id DESC LIMIT $offset, $perPage";
		$sql = $pdo->prepare($sql);
	...
	public function editarAnuncio($titulo, $categoria, $valor, $descricao, $estado, $fotos, $id)
	{
		...
		$totalFotos = count($fotos['tmp_name']);		
		if($totalFotos > 0) 
		{			
			for($q = 0; $q < $totalFotos; $q++)
			{
				$tipo = $fotos['type'][$q];
	...
	public function excluirFoto($id)
	{
		...
		if($sql->rowCount() > 0) 
		{
			$id_anuncio = $sql->fetch()['id_anuncio'];
		}
